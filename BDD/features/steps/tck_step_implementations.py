...
from up_client_socket_python.utils.constants import SEND_COMMAND, \
    REGISTER_LISTENER_COMMAND, UNREGISTER_LISTENER_COMMAND, \
    INVOKE_METHOD_COMMAND, LONG_URI_SERIALIZE, LONG_URI_DESERIALIZE, \
    MICRO_URI_SERIALIZE, MICRO_URI_DESERIALIZE
...
def step_impl(context, sdk_name: str, command: str):
    ...
    context.json_array = {}
    if command == "longuriserialize" or command == "microuriserialize":
...
def step_impl(context: Context, key: str, value: str):
    ...
    '''
    context.logger.info(
        "Json data: Key is " + str(key) + " value is " + str(value))
    if key not in context.json_array:
        if key == "serializeddata":
            context.json_array[key] = [context.serialized_data]
        else:
...
def step_impl(context, command: str):
    context.logger.info(
        f"Json request for {command} -> {str(context.json_array)}")
    if command == "longuriserialize" or command == "microuriserialize":
        context.serialized_data = context.tm.json_action_request(
            context.json_array)
        context.logger.info(
            f"Serialized data received: {context.serialized_data}")
    else:
        context.status = context.tm.json_action_request(context.json_array)
        context.logger.info(f"Status Received: {context.status}")


@when(u'“{ue}” sends request to "{command}" using the serialized data')
def step_impl(context, ue, command):
    context.json_array = {}
    context.json_array['ue'] = [ue]
    context.json_array['action'] = [command]

    if context.serialized_data not in ['', None]:
        context.deserialized_data = context.tm.json_action_request(
            context.json_array)
        context.logger.info(
            f"Deserialized data received: {context.deserialized_data}")
    else:
        raise ValueError(
            "Serialized Data is empty or Null. Cannot send deserialize request")


@then(u'deserialized data from “{ue2}” is equal to data created by “{ue1}”')
def step_impl(context, ue2, ue1):
    if context.deserialized_data not in ['', None]:
        context.logger.info(f"Deserialized Data: {context.deserialized_data}")
        #TODO:Validation for received deserialized data to be added.
    else:
        raise ValueError("DeSerialized Data is empty or Null")
...
def step_impl(context, sdk_name, key, value):
    try:
        if context.tm.received_umessage not in ['', None]:
            ...
            context.logger.info(
                f"Payload data for {sdk_name} is {received_payload}")
            assert_that(received_payload.value.decode('utf-8'),
                        equal_to(value))
        else:
...
@step(u'"{sdk_name}" receives serialized data')
def step_impl(context, sdk_name):
    try:
        if context.serialized_data not in ['', None]:
            context.logger.info(
                f"Received serialized Data as {context.serialized_data}")
            assert_that(context.serialized_data, not (equal_to('')))
        else:
            raise ValueError(f"Received empty data for {sdk_name}")

    except AssertionError as ae:
        raise AssertionError("Assertion error. Expected some serialied data "
                             f"but received {context.serialized_data}: {ae}")
...
def tm_connects_to_ta_socket(context, sdk_name: str):
    ...
    context.logger.info(
        f"{sdk_name} TA connects to TM {test_manager.sdk_to_test_agent_socket.keys()}")
...
def tm_closing_ta_socket(context, sdk_name: str):
    ...
    context.logger.info(
        f"TM closed TA connection {sdk_name} {test_manager.sdk_to_test_agent_socket.keys()}")
    pass
...
@given(
    'protobuf UEntity "{entity}" sets parameter "{param}" equal to string "{value}"')
...
@given(
    'protobuf UResource "{resrc}" sets parameter "{param}" equal to string "{value}"')
...
@given(
    'protobuf UUri "{uuri}" sets parameter "{param}" equal to created protobuf "{value}"')
def initialize_protobuf(context, uuri: str, param: str, value: str):
    if uuri not in context.initialized_data:
        ...
        context.initialized_data["uuri_builder"] = context.initialized_data[
            uuri]
...
@given(
    'protobuf UAttributes "{uattr}" creates publish message with parameter source equal to created protobuf "{value}"')
def initialize_protobuf(context, uattr: str, value: str):
    value_builder: Builder = context.initialized_data[value]

    new_builder: UAttributesBuilder = UAttributesBuilder.publish(
        value_builder.build(), UPriority.UPRIORITY_CS1)
    context.initialized_data[uattr] = new_builder
    context.initialized_data["uattributes_builder"] = new_builder


@given(
    'protobuf UPayload "{payload}" sets parameter "{param}" equal to "{value}"')
def initialize_protobuf(context, payload: str, param: str, value: str):
    if payload not in context.initialized_data:
        ...
        context.initialized_data["upayload_builder"] = \
            context.initialized_data[payload]
...
def tm_sends_request(context, command: str, sdk_name: str):
    ...
    if command == REGISTER_LISTENER_COMMAND:
        ...
        uattr_builder: UAttributesBuilder = context.initialized_data[
            "uattributes_builder"]
        uattr: UAttributes = uattr_builder.build()

        upay_builder: UPayloadBuilder = context.initialized_data[
            "upayload_builder"]
        upayload: UPayload = upay_builder.build()

        umsg: UMessage = UMessageBuilder().set_uuri(uuri).set_payload(
            upayload).set_attributes(uattr).build()
...
@when(
    'Test Manager sends "{command}" uri serializer request to Test Agent "{sdk_name}"')
def tm_sends_uri_serializer_request(context, command: str, sdk_name: str):
    ...
    if command == LONG_URI_SERIALIZE:
        translation: str = test_manager.uriserializer_request(sdk_name,
                                                              command, uuri)
        ...
        translation: UUri = test_manager.uriserializer_request(sdk_name,
                                                               command,
                                                               topic_str)
...
@then(
    'Test Manager receives an "{status_code}" status for "{command}" request')
def tm_receives_response(context, status_code: str, command: str):
    response_status: UStatus = context.status
    if response_status is None:
        raise ValueError(
            f"{command} request did not receive a response UStatus")
...
@then(
    'Test Manager receives OnReceive UMessage from "{sdk_name}" Test Agent with parameter UPayload "{param}" with parameter "{inner_param}" as "{expected}"')
def tm_receives_onreceive(context, sdk_name: str, param: str, inner_param: str,
                          expected: str):
    test_manager: SocketTestManager = context.tm
...
def tm_receives_serializer_translation(context, expected_translation: str):
    if context.translation is None:
        raise ValueError(
            f"Test Manager did not receive any serializer translation")
